...
If you want to run locally, use Docker. To know how to start the containers, consider reading the last lines of `infrastructure/hetzner/startup_script.sh` and read the `docker-compose.yml` file to know how the volumes are expected to be.
...
NOTE: You should set `ARBITRARY_GOS_BUILD_NUMBER` to the release you want when you want to reproduce an older specific release instead of defaulting to the latest one.
