...
for GOS_RELEASE_CHANNEL in $GOS_RELEASE_CHANNELS; do
  for PIXEL_CODENAME in $PIXEL_CODENAMES; do
    read -r GOS_BUILD_NUMBER GOS_BUILD_DATETIME BUILD_ID _ < <(echo $(curl -sL "https://releases.grapheneos.org/${PIXEL_CODENAME}-${GOS_RELEASE_CHANNEL}"))
