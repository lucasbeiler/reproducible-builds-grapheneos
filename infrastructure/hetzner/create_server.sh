...
export NONROOT_USER HETZNER_LOCATION HETZNER_API_TOKEN AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_DEFAULT_REGION AWS_BUCKET_NAME GOS_BUILD_NUMBER GOS_BUILD_DATETIME
...
export EXTRACT_INFO_FROM_OFFICIAL_BUILD_B64=$(cat "../../scripts/extract_info_from_official_build.sh" | gzip | base64 | tr -d '\n')
...
for PIXEL_CODENAME in $PIXEL_CODENAMES; do
  ...
  if [[ "$response_status_code" -ge 400 && "$response_status_code" -lt 500 || $FORCE_REPEAT_IF_ALREADY_REPRODUCED == true ]]; then
    ...
    curl -sL ${CURL_OUTPUT} \
        ...
        -d "{\"user_data\":\"$USER_DATA\",\"ssh_keys\":[\"main_one\"],\"image\":\"debian-12\",\"location\":\"${HETZNER_LOCATION}\",\"labels\":{\"pixel_codename\":\"$PIXEL_CODENAME\",\"gos_build_number\":\"$GOS_BUILD_NUMBER\"},\"name\":\"m-$(date +%s)\",\"public_net\":{\"enable_ipv4\":true,\"enable_ipv6\":true},\"server_type\":\"cpx51\",\"start_after_create\":true}" \
