...
# Build and install payload-dumper-go. I'm already preparing right here to fail early if it fails.
git clone https://github.com/ssut/payload-dumper-go ~/payload-dumper-go
cd ~/payload-dumper-go
go build

# Download official builds to ~/comparing/official. I'm already preparing right here to fail early if it fails.
...
####### Now, let's download vendor blobs and build kernels, etc, in order to replace the prebuilts from the GrapheneOS tree.
...
cd ~/grapheneos/grapheneos-${GOS_BUILD_NUMBER}
# TODO: Improve this copy command.
...
cd ~/grapheneos/grapheneos-${GOS_BUILD_NUMBER}
# TODO: Improve these copy commands.
