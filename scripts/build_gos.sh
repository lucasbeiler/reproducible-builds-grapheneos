...
# Force localversion string and build timestamp to values obtained from the official build, in order to workaround some issues.
echo "Forcing kernel build localversion string and build timestamp to, respectively: -${KERNEL_BUILD_STRING#*-} and ${KERNEL_BUILD_TIMESTAMP_EPOCH}"
cd aosp
echo "-${KERNEL_BUILD_STRING#*-}" > .scmversion
sed -i "s/^echo \"\$res\"/echo -${KERNEL_BUILD_STRING#*-}/g" scripts/setlocalversion
