...
go build
export PATH="$PATH:$HOME/payload-dumper-go"
...
sha512sum official/*.zip > ~/comparing/diffoscope_outputs/${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.checksums
...
for i in $(seq 4); do
  ...
  for apex in $(find -type f -name '*.apex'); do
    ...
    echo "${apex}" >> diffoscope_outputs/deleted-apexes-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt # TODO: Remove this debugging file when confident enough.
    rm -f ${apex}
  done;
...
find ~/comparing/ -type f > ~/comparing/diffoscope_outputs/find-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
