...
for sha256sums_file in ~/comparing/operation_outputs/*_ota_payload_images.sha256sums.txt; do
  if [[ -f "$sha256sums_file" ]]; then
    ...
    if [ "$current_hash" != "$first_hash" ]; then
      echo "Data in the official full OTA and incrementals/deltas do not match!" > ~/comparing/operation_outputs/ota-partitions-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
      echo -e "\n\nCheck the hashes below:" >> ~/comparing/operation_outputs/ota-partitions-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
      cat ~/comparing/operation_outputs/*_ota_payload_images.sha256sums.txt | sort | uniq >> ~/comparing/operation_outputs/ota-partitions-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
      exit
    fi
  fi
done

echo "The target hashes of the partitions after the incremental/delta operations for all incremental/delta images match the hashes of the partitions in the full OTA image." > ~/comparing/operation_outputs/ota-partitions-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
echo "The official full OTA image was compared with my reproduced build and now its partitions' hashes were compared with the target hashes of the operations in the official incrementals/deltas targets, so there are chained guarantees that the official incrementals are also trustworthy." >> ~/comparing/operation_outputs/ota-partitions-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
echo -e "\n\nCheck the hashes below:" >> ~/comparing/operation_outputs/ota-partitions-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
cat ~/comparing/operation_outputs/*_ota_payload_images.sha256sums.txt | sort | uniq >> ~/comparing/operation_outputs/ota-partitions-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
