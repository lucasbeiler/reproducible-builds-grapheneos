...
# Move the binaries of AOSP-related utilities somewhere else before deleting the source tree.
mv ~/grapheneos/grapheneos-${GOS_BUILD_NUMBER}/out/ ~/aosp-bins/

# Prepare working environment.
export PATH="$PATH:$HOME/aosp-bins/host/linux-x86/bin/:/sbin:/usr/sbin:/usr/local/sbin"

# Delete OS source tree.
rm -rf ~/grapheneos/

# Save the verified boot hash of the official build.
extract_vbmeta_hashes_to_file ~/comparing/official/${PIXEL_CODENAME}-install-${GOS_BUILD_NUMBER}.zip
...
for i in $(seq 10); do
  ...
  # TODO: Improve this.
  for kernel in $(find -type f -name 'kernel' ! -empty); do
    ...
    if [[ "$(file ${kernel})" =~ 'Linux kernel ARM64 boot executable Image' ]]; then
      if binwalk "${kernel}" | grep -i 'certificate in der format'; then
        read -r byte_sign_start byte_sign_length < <(echo $(binwalk "${kernel}" | grep -i 'certificate in der format' |  head -n1 | awk '{print $1, $NF}'))
...
find -name '*.ko' | xargs aarch64-linux-gnu-strip --remove-section=.comment --remove-section=.note --remove-section=.BTF --remove-section=.note.gnu.build-id --remove-section=.modinfo --remove-section=.llvm_addrsig
...
find ~/comparing/ -type f | gzip > ~/comparing/operation_outputs/debug-find-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
...
grep -viE "Successfully verified|Verifying image|([a-fA-F0-9]{64})" ~/*.vbmeta_results.txt && echo "Something wrong with vbmeta checks." > ~/comparing/operation_outputs/avb-hash-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
echo "Verified Boot hash of the official build that was tested: $(grep -iE '([a-fA-F0-9]{64})' ~/*.vbmeta_results.txt | sort | uniq)" >> ~/comparing/operation_outputs/avb-hash-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
echo -e "\nYou can compare it with the Verified Boot hash attested by Auditor on your device." >> ~/comparing/operation_outputs/avb-hash-${PIXEL_CODENAME}-${GOS_BUILD_NUMBER}.txt
