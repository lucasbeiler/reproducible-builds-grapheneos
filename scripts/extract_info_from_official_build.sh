...
# Prepare directories and download official builds to ~/comparing/official.
...
# Prepare things.
TMP_DIR=$(mktemp -d)
unzip ~/comparing/official/${PIXEL_CODENAME}-install-${GOS_BUILD_NUMBER}.zip */*.img -d "${TMP_DIR}"
