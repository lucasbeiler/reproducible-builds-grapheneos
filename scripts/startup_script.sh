...
apt --install-recommends install -y python3-protobuf diffoscope dexdump enjarify aapt androguard apksigcopier apksigner apktool e2fsprogs python3-pip golang binwalk device-tree-compiler awscli liblzma-dev lz4 binutils-aarch64-linux-gnu xxd android-sdk-libsparse-utils erofs-utils
echo "deb http://deb.debian.org/debian bookworm-backports main" > /etc/apt/sources.list.d/bw-bps.list
apt update
apt install -y -t bookworm-backports 7zip
...
### No more swap as I've got more RAM and there's plenty of RAM now.
## Prepare swapfile.
